...
async function authenticate() {
    ...
    // Try environment variable first (for Koyeb/Render)
    ...
    if (!credentials) {
        throw new Error('No Google credentials found. Set GOOGLE_CREDENTIALS_JSON env var or provide a credentials file.');
    }
...
async function readTokensAndPhones(startRow, endRow) {
    const authClient = await authenticate();
    const sheets = google.sheets({ version: 'v4', auth: authClient });
    const range = `${SHEET_NAME}!B${startRow}:C${endRow}`;
    ...
    console.log(`📋 Loaded ${data.length} valid rows (${startRow} – ${startRow + rows.length - 1})`);
    return data;
}
...
async function readConfig() {
    try {
        ...
        return {
            startRow: parseInt(config.START_ROW) || DEFAULT_START_ROW,
            endRow: parseInt(config.END_ROW) || DEFAULT_END_ROW,
            username: config.USERNAME || DEFAULT_USERNAME,
            batchSize: parseInt(config.BATCH_SIZE) || 10,
            staggerMs: parseInt(config.STAGGER_MS) || 3000,
        };
    } catch (error) {
        console.log('⚠️ Could not read config, using defaults:', error.message);
        return {
...
// ==================== LOGIN STEPS ====================
async function step1_InitialRequest(phoneNumber, accessToken, username) {
    ...
    const response = await makeRequest(url, {
        headers: {
            ...
            'avatar': '', 'lang': LANG, 'username': username || DEFAULT_USERNAME,
...
async function step2_FollowRedirect(redirectUrl, phoneNumber, accessToken, username) {
...
function playEndlessGame(baseUrl, gameToken, gameMode = { level: 1, story: 0, mode: 'ENDLESS' }) {
    return new Promise((resolve) => {
        ...
        ws.on('open', () => {
            ...
            ws.send(JSON.stringify({ type: 'START_GAME', payload: { data: encryptedData } }));
...
function sendGroundSelected(ws) {
...
async function processSingleAccount(user, currentUsername) {
    const { rowNumber, accessToken, phone } = user;
    const username = currentUsername || DEFAULT_USERNAME;
    
    try {
        const step1 = await step1_InitialRequest(phone, accessToken, username);
        await step2_FollowRedirect(step1.redirectUrl, phone, accessToken, username);
        ...
        while (totalTickets > 0) {
            ...
            if (gamesPlayed % 20 === 1 && gamesPlayed > 1) {
                try {
                    const ns1 = await step1_InitialRequest(phone, accessToken, username);
        ...
        console.log(`  [${phone}] ✅ ${gamesPlayed} games, ${totalBananasCollected}🍌, ${totalBoxes}📦`);
...
async function main() {
    ...
    const CURRENT_USERNAME = config.username;
    const CURRENT_START_ROW = config.startRow;
    const CURRENT_END_ROW = config.endRow;
    const CURRENT_BATCH_SIZE = config.batchSize;
    const CURRENT_STAGGER_MS = config.staggerMs;
    
    console.log(`📊 Sheets: ${SHEET_NAME} | Rows: ${CURRENT_START_ROW}-${CURRENT_END_ROW}`);
    console.log(`📦 Batch Size: ${CURRENT_BATCH_SIZE} | Stagger: ${CURRENT_STAGGER_MS}ms`);
    console.log(`👤 Username: ${CURRENT_USERNAME}\n`);
    
    const users = await readTokensAndPhones(CURRENT_START_ROW, CURRENT_END_ROW);
    ...
    const batches = [];
    for (let i = 0; i < users.length; i += CURRENT_BATCH_SIZE) {
        batches.push(users.slice(i, i + CURRENT_BATCH_SIZE));
    }
    ...
    for (let batchIdx = 0; batchIdx < batches.length; batchIdx++) {
        ...
        const promises = batch.map((row, index) => {
            return new Promise(async (resolve) => {
                await sleep(index * CURRENT_STAGGER_MS);
                const result = await processSingleAccount(row, CURRENT_USERNAME);
                resolve(result);
            });
        });
